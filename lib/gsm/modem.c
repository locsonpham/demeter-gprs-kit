...
uint8_t cellIDInitFlag = 0;
uint8_t cellIDActive = 0;
...
//     delay_ms(500);
    ...
    if(MODEM_SendCommand(modemOk,modemError, 15000, 1,"AT+SAPBR=1,1\r") == 0xff) return 0xff;
    delay_ms(500);
    // Get IP context
...
uint8_t MODEM_EnableGPRS(void)
{
    ...
    delay_ms(TIME_SEC(10)); // delay time GPRS ready connect
    
    // Get Current IP
    MODEM_Info("\r\nGSM->LOCAL IP\r\n");
	COMM_Puts("AT+CIFSR\r");
	if(MODEM_CheckResponse("\r", 10000)){
		MODEM_Info("\r\nGSM->LOCAL IP FAIL\r\n");
		return 0xff;
	}
